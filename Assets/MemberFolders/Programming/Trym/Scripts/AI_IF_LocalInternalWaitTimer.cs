...
public class AI_IF_LocalInternalWaitTimer : StateBlock
{
    ...
    private readonly Dictionary<int,float> _stopTimes = new();
    
    public override void OnEnd(EnemyController user, GameObject target)
    {
        int id = user.GetInstanceID();
        if (!_stopTimes.ContainsKey(id))
        {
            _stopTimes.Add(id, Time.time);
        }
    ...
    private void Cleanup(EnemyController obj)
    {
        ...
        _stopTimes.Remove(obj.GetInstanceID());
    }
    ...
    public override (AI_State state, List<float> val) OnUpdate(EnemyController user, GameObject target)
    {
        ...
        {
            if ((_time - (Time.time - (_startTimes[id] - (_stopTimes.ContainsKey(id)?Time.time - _stopTimes[id]:0))) <= 0) == !_invert)
            {
                if (!_invert)
                {
